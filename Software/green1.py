...
@author: akshitshishodia, adityapaul, rohakjain
"""
#import sys
import numpy as np
import cv2
...
while(True):
    ...
    ret, frame = vid.read()
    frame = cv2.resize(frame, (300, int(
        frame.shape[0]*(300/frame.shape[1]))), interpolation=cv2.INTER_AREA)
    ...
    for j in range(4):
        ...
        c, hierarachy = cv2.findContours(
            edged1, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if(len(c) != 0):
    ...
    im_f = cv2.vconcat([cv2.hconcat([slice_part[0], slice_part[1]]),
                       cv2.hconcat([slice_part[2], slice_part[3]])])
